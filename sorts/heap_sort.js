...
/* define function heapify that takes an input array and index, compares index with 
values 2i+1 and 2i+2 away from it to find the largest value. If the index of 
largest value is not the input index, swap the largest and input values, 
and call heapify function itself with input array and the largest index as parameters */
function heapify(input, i) {
    ...
    /* if input value at left index is larger than value at array length && the index itself 
    is smaller than array length, set largest as left */
...
/* define function heap_sort that builds the initial heap, and for the length - 1 of input array, 
swap the first and current element within the loop, reduce array length, and heapify the input 
at the first index */
