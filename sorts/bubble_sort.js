...
/* Function bubble_sort: iterate through the input_array, compare an element and one after that, 
if right element is less than left one, swap, iterate until all values are swapped */
function bubble_sort( input_array ) {
	...
	let swapped = true;
	/* Loop through the input_array, compare each value with the one on the right, 
	swap if right is less than left. If a swap has occured during each loop cycle, 
	repeat until no swap has occured. */
	while ( swapped ) {
		swapped = false;
